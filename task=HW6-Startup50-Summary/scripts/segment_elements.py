...
# --- 50_Startups deck overrides ---
# Slide 10 & 11 are line charts. The user wants the whole chart figure to
# reveal as ONE unit first, then the red circle / arrow / callout remarks
# fade in afterwards.
#   Slide 10: the chart was split into axis-label + ticks + curves, with the
#     x-axis ("Number of Features" 2-6) stranded in the red-remark piece.
#     Merge the whole figure region (both axes + curves + the red marks) into
#     one `chart` layer. The black axes/curves stay as the chart base; the
#     red-annotation pass lifts the red circle, arrow and words out as
#     `annotation` layers that fade in after the whole chart.
#   Slide 11: the chart was already one piece but the red "最高峰值" callout
#     animated BEFORE it. Extend the chart box upward to include the callout so
#     the red-annotation pass lifts callout + arrow note + star out as
#     `annotation` layers, which fade in 0.85s AFTER the whole chart.
OVERRIDES = {
    10: {"merge": [{"box": [332, 282, 1149, 770], "type": "chart"}]},
    11: {"merge": [{"box": [124, 334, 1704, 727], "type": "chart"}]},
}
